...
public abstract class AbstractSearchPage extends Page {
    ...
    public void setPermissions(String permissions) {
        int readPermission = 4, writePermission = 2, executePermission = 1;

        String[] permissionArray = permissions.split("x")[1].split("(?!^)");
        int ownerPermission = Integer.parseInt(permissionArray[0]);
        int groupPermission = Integer.parseInt(permissionArray[1]);
        int othersPermission = Integer.parseInt(permissionArray[2]);

        List<WebElement> aclOwnerElements = getACLOwner();
        List<WebElement> aclGroupElements = getACLGroup();
        List<WebElement> aclOthersElements = getACLOthers();
        // Clear the permission checkboxes if checked
        for (WebElement aclOwnerElement : aclOwnerElements) {
            if (aclOwnerElement.isSelected()) {
                aclOwnerElement.click();
            }
        }
        for (WebElement aclGroupElement : aclGroupElements) {
            if (aclGroupElement.isSelected()) {
                aclGroupElement.click();
            }
        }
        for (WebElement aclOthersElement : aclOthersElements) {
            if (aclOthersElement.isSelected()) {
                aclOthersElement.click();
            }
        }

        if ((ownerPermission & readPermission) == readPermission){
            aclOwnerElements.get(0).click();
        }
        if ((ownerPermission & writePermission) == writePermission){
            aclOwnerElements.get(1).click();
        }
        if ((ownerPermission & executePermission) == executePermission){
            aclOwnerElements.get(2).click();
        }
        if ((groupPermission & readPermission) == readPermission){
            aclGroupElements.get(0).click();
        }
        if ((groupPermission & writePermission) == writePermission){
            aclGroupElements.get(1).click();
        }
        if ((groupPermission & executePermission) == executePermission){
            aclGroupElements.get(2).click();
        }
        if ((othersPermission & readPermission) == readPermission){
            aclOthersElements.get(0).click();
        }
        if ((othersPermission & writePermission) == writePermission){
            aclOthersElements.get(1).click();
        }
        if ((othersPermission & executePermission) == executePermission){
            aclOthersElements.get(2).click();
        }
    }

    private List<WebElement> getACLOwner(){
        return driver.findElements(By.xpath("//input[@name='owner']"));
    }

    private List<WebElement> getACLGroup(){
        return driver.findElements(By.xpath("//input[@name='groups']"));
    }

    private List<WebElement> getACLOthers(){
        return driver.findElements(By.xpath("//input[@name='others']"));
    }
